...
void snd_pump(Cpu& cpu) {
    ...
    for (auto& db : s_dblbufs) {
        ...
        for (int fired = 0; fired < fired_cap; fired++) {
            ...
            // decoder block, decays during gameplay). Render ahead only while it wants
            // more, checked EVERY iteration (NOT just fired>0: snd_pump is called
            // thousands of times/sec via the VBL poll traps, so forcing one buffer per
            // call races the schedule head seconds into the future -> silence).
            (void)bytes;
            if (!pop2_audio_want_more()) break;
#else
